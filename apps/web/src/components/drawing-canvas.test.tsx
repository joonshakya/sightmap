...
describe('DrawingCanvas', () => {
  ...
  test('exposes ref method startPathCreation', () => {
    ...
    expect(() => ref.current.startPathCreation('room1')).not.toThrow()
  })

  test('handles mode switching between room/path modes', () => {
    // Test room mode
    const { rerender } = render(<DrawingCanvas {...defaultProps} mode="room" />)
    expect(screen.getByTestId('konva-stage')).toBeInTheDocument()

    // Test path mode
    rerender(<DrawingCanvas {...defaultProps} mode="path" />)
    expect(screen.getByTestId('konva-stage')).toBeInTheDocument()
  ...
  test('handles different prop configurations', () => {
    ...
    const { rerender } = render(<DrawingCanvas {...propsWithGridSize} />)
    expect(screen.getByTestId('konva-stage')).toBeInTheDocument()
