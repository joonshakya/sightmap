...
import type { PathAnchor, Room } from '@sightmap/common/prisma/client'
...
describe('Instruction Generation Functions', () => {
  ...
  describe('calculatePathSegments', () => {
    test('calculates segments with steps and directions', () => {
      const anchors: PathAnchor[] = [
        { id: '1', pathId: 'path1', index: 0, xCoords: 0, yCoords: 0 } as PathAnchor,
        { id: '2', pathId: 'path1', index: 1, xCoords: 50, yCoords: 0 } as PathAnchor, // 50 right
        { id: '3', pathId: 'path1', index: 2, xCoords: 50, yCoords: 100 } as PathAnchor, // 100 down
      ]

      const rooms: Room[] = [
        { id: 'room1', name: 'Classroom A', number: '101', floorId: 'floor1', x: 40, y: 20, width: 20, height: 20 } as Room,
      ]
  ...
  describe('calculateRelativeDirections', () => {
    test('adds relative directions and facing info', () => {
      const segments = [
        { direction: 'forward' as const, steps: 5, nearbyRooms: [] },
        { direction: 'right' as const, steps: 3, nearbyRooms: ['Lab'] },
        { direction: 'forward' as const, steps: 2, nearbyRooms: [] },
      ]
    ...
    test('handles turn left', () => {
      const segments = [
        { direction: 'forward' as const, steps: 1, nearbyRooms: [] },
        { direction: 'left' as const, steps: 2, nearbyRooms: [] },
      ]

      const result = calculateRelativeDirections(segments)

      expect(result[1].relativeDirection).toBe('Turn left and move forward')
    })
  })
  ...
  describe('Integration - calculatePathSegments and calculateRelativeDirections', () => {
    test('processes complete path from anchors to relative directions', () => {
      const anchors: PathAnchor[] = [
        { id: '1', pathId: 'path1', index: 0, xCoords: 0, yCoords: 0 } as PathAnchor,
        { id: '2', pathId: 'path1', index: 1, xCoords: 100, yCoords: 0 } as PathAnchor, // move right 100
        { id: '3', pathId: 'path1', index: 2, xCoords: 100, yCoords: 150 } as PathAnchor, // move down 150
      ]

      const rooms: Room[] = []

      const pathSegments = calculatePathSegments(anchors, rooms)
      const relativeSegments = calculateRelativeDirections(pathSegments)

      expect(relativeSegments).toHaveLength(2)
      expect(relativeSegments[0].relativeDirection).toBe('Move forward')
      expect(relativeSegments[0].steps).toBe(5) // 100 / 20 = 5

      expect(relativeSegments[1].relativeDirection).toBe('Turn right and move forward')
      expect(relativeSegments[1].steps).toBe(8) // 150 / 20 = 7.5, rounded to 8
    })

    test('includes nearby rooms information in output', () => {
      const anchors: PathAnchor[] = [
        { id: '1', pathId: 'path1', index: 0, xCoords: 100, yCoords: 100 } as PathAnchor,
        { id: '2', pathId: 'path1', index: 1, xCoords: 200, yCoords: 100 } as PathAnchor,
      ]

      const rooms: Room[] = [
        { id: 'room1', name: 'Library', number: '200', floorId: 'floor1', x: 150, y: 80, width: 40, height: 40 } as Room,
      ]
